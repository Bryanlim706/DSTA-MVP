...
import type { MappedRequirement, Step6Result, Step7Result } from '../types'
import { MappedRequirementList } from './MappingResult'

interface Props {
  result: Step7Result | null
  loading: boolean
  step6Result?: Step6Result | null
  l1aIds?: Set<string>
}
...
export default function ScoringResult({ result, loading, step6Result, l1aIds }: Props) {
  const [showFcomAdvisory, setShowFcomAdvisory] = useState(false)
  const [showFaAdvisory, setShowFaAdvisory] = useState(false)

  const fcomMapped: MappedRequirement[] = step6Result?.mapped.filter(r => l1aIds?.has(r.req_id)) ?? []
  const faMapped: MappedRequirement[] = step6Result?.mapped.filter(r => !l1aIds?.has(r.req_id)) ?? []
    ...
    return (
      <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-gray-900">Mapping entities &amp; computing scores…</p>
  ...
  if (result.error) {
    return (
      <div className="bg-white border border-red-200 rounded-xl p-6 shadow-sm">
        <p className="text-sm font-semibold text-gray-900">Scorer</p>
  ...
  const hasFaAdvisory = faMapped.length > 0
    ...
    <>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm space-y-5">
        ...
        <div className="flex gap-4">
          <ScorePanel score={result.fcom} detail={result.fcom_detail} />
        </div>
          ...
          <div className="border border-gray-200 rounded-lg overflow-hidden">
            ...
            {showFcomAdvisory && (
              <div className="px-4 py-4 space-y-4">
                <MappedRequirementList reqs={fcomMapped} />
                <UnlinkedSection
          ...
          <div className="border border-gray-200 rounded-lg overflow-hidden">
            ...
            {showFaAdvisory && (
              <div className="px-4 py-4 space-y-4">
                ...
                <MappedRequirementList reqs={faMapped} />
              </div>
            )}
          </div>
        )}
