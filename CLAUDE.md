...
- **`route_to_files`:** maps each frontend route → source file(s). File-based routers (Next.js, SvelteKit) get exact 1:1 mapping. React Router gets the JSX file containing `<Route>` definitions; routes not explicitly mapped fall back to `important_files[:5]`. Consumed by Step 5 static fallback.
- **`implementation_units`:** unified list of backend handlers — wraps all `api_endpoints` as `kind: "api_endpoint"` plus HTML `<form method="POST/PUT/DELETE">` tags in SSR template files as `kind: "form_handler"`. This is the authoritative L3 signal for Step 6 action edge E() scoring (covers both REST APIs and SSR form submissions).
...
- **`route_to_files`** — Step 4 extra field (beyond PLAN.md minimum schema) mapping each frontend route to its source file(s). File-based routers (Next.js pages/, SvelteKit +page.svelte) produce exact 1:1 maps. React Router apps: `_route_component_files()` parses `import X from "..."` + `element={<X />}` in the router file to resolve each route → component file; remaining routes fall back to `important_files[:5]`. Step 5 reads this for static fallback: when Playwright can't visit a route, parse the component source file rather than the router file (which has no form elements).
...
- **Step 5 is a passive crawl only** — Playwright visits each route from Step 4 on page load, extracts visible elements with selectors, and records outbound links. It does NOT fill forms or click buttons — form-submission POST/PUT/DELETE calls are never captured in `api_calls_observed`. `api_calls_observed` is supplementary cross-check against Step 4 only, not an E() evidence source. Bootstrap is heuristic-based from Step 0 `project_context`: `frontend_only`/Vite → `npm run dev`; FastAPI → `uvicorn main:app`; etc. Static Tree-sitter fallback for auth-gated routes. CSS selectors come from the running Playwright DOM (not from source analysis — CSS-in-JS and component libraries make source selectors unreliable). Step 9 does NOT read Step 5 selectors; Step 9 generates `getByRole`/`getByText`/`getByPlaceholder` locators from path entity labels via LLM.
- **Step 6 scores E() per entity, not per requirement** — Each `path[]` entity gets its own E() value via a piecewise function by entity type. `node` entities: route in Step 4 + page accessible in Step 5 → {1.0, 0.5, 0.0}. `element` entities: element found in Step 5 (Playwright=1.0, static fallback=0.5, not found=0.0) via LLM fuzzy label matching. `action edge` entities (submit/create/delete/update implied): endpoint in Step 4 + triggering element in Step 5 → {1.0, 0.5, 0.4, 0.0}. `navigation edge` entities: NOT scored (outbound link detection unreliable for programmatic navigation). Aggregated via formula: `E(req) = 0.7×[primary avg] + 0.3×[secondary avg]` excluding navigation edges.
...
- **Unlinked detection (Steps 5/6)** — unlinked L2 = Step 5 routes visited by Playwright where no L1a path[] node entity matched them; unlinked L3 = Step 4 api_endpoints not matched as the L3 signal for any L1a requirement. Route-level and endpoint-level, not named-function-level. Step 7 advisory reports both lists.
- **Step 4/5 known limitations (by framework)** — Laravel: no PHP/web.php route parser or Eloquent model extractor; `frontend_routes=[]`, Step 5 returns "No frontend routes" non-fatal error. React SPA / Electron (no router, single-page): `frontend_routes=['/']`, static fallback reads App component but gets 0 elements if the App is a layout wrapper (all UI is in child components not traversable without full import resolution or Playwright). Flask SSR `route_to_files['/']`: only contains `app.py` when the root template is named anything other than `index.html`; Step 5 reads Python source (no elements). None of these are bugs — they are the limit of static analysis without running the app.
