...
- **Step 5 is a passive crawl only** — Playwright visits each route from Step 4 on page load, extracts visible elements with selectors, and records outbound links. It does NOT fill forms or click buttons — form-submission POST/PUT/DELETE calls are never captured in `api_calls_observed`. `api_calls_observed` is therefore only a supplementary cross-check against Step 4, not an L2 evidence source for E() scoring.
- **Step 6 scores E() at requirement level, not entity level** — Path entities from L1a are a search specification (guide Step 6 to the right page, tell it what element types to look for) but not the atomic scoring unit. Scoring entity-by-entity is unreliable because LLM-generated entity labels from Step 1 may not match Playwright-found labels. Instead, Step 6 computes two signals per requirement: (1) L2 score — fraction of primary element entities found on the matched page via LLM fuzzy matching, with auth-gated pages discounted × 0.5; (2) L3 score — binary, does Step 4 have the endpoint implied by this requirement's action. E() = {1.0 if L2 high + L3 found / 0.5 if L2 low + L3 found / 0.4 if L2 high + no L3 / 0.0 if both low}. The α=0.7/0.3 primary/secondary weighting from the Conceptual Model is applied inside the L2 signal computation (primary elements count 70%, secondary 30%), not across two separate E() values.
