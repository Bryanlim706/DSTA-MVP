...
- **Path construction:** Every generated function has a complete `path[]` with entry edge, body entities, and exit edge baked in. No `structural_edge` category — entry/exit are part of the function's path. State-variant nodes must be omitted entirely (not just marked `primary: false`) — path terminates at the last interaction element or edge.
...
- **Step 11 Docker boot strategy** — Primary stack: Spring Boot (Maven/Gradle) + React (Vite/CRA/Angular/Next.js). Full boot pipeline:
  ...
  - **Vite proxy patching:** `_patch_vite_config(sb_frontend, spring_port, inject_proxy)` — (1) rewrites any `http://localhost:8xxx/9xxx` in existing proxy entries to `http://backend:{spring_port}`; (2) when `inject_proxy=True` (api_style="relative") and no proxy block exists, injects a catch-all `'/'` proxy with HTML bypass after `defineConfig({` — so `fetch('/api/...')` calls are forwarded to the backend container.
  - **Compose port logic:** `_compose_yaml()` receives `spring_port`, `api_style`, `hardcoded_api_port`. Always exposes `BACKEND_HOST_PORT(8081):{spring_port}` for health polling. For `api_style="hardcoded"`: also exposes `{hardcoded_api_port}:{spring_port}` so the browser's direct call resolves. No extra port for relative/env_based. Removed the old `"8080:8080"` workaround.
  - **Dockerfiles:** Backend: Maven multi-stage (`maven:3.9-eclipse-temurin-21-alpine` build stage + `eclipse-temurin:21-jre-alpine` runtime); fat-JAR detected via `jar tf "$j" | grep "^BOOT-INF/"`. Gradle: prefers `./gradlew bootJar`. Frontend: 5 templates — Vite (VITE_API_URL build arg), CRA (PORT=5174, REACT_APP_API_URL), Angular (npx ng serve --port 5174), Next.js (PORT=5174, NEXT_PUBLIC_API_URL), Generic. All standardise on internal port 5174.
  ...
  - **Output adds** `db_type`, `frontend_type`, `spring_port`, `api_style` fields to Step 11 result schema.
