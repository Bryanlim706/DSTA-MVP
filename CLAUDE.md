...
- **Three-pass Step 3** — SOP (pattern table fires on Step 1 stated requirements) + INF (pure domain reasoning from `project_summary`) + a dedicated **Pass 3 deduplication** LLM call. No 5-category taxonomy. No `structural_edge` — entry/exit baked into function paths. `placement` replaces `l1_recommendation`. Step 3 does not re-apply Step 2 connectivity checks. **Generation passes carry NO dedup prose** — the in-prompt dedup rules were removed because they were pass-scoped (siloed in Pass 1, absent from Pass 2), contradicted by the pattern table, and unable to catch cross-list (generated-vs-obvious) or generated-vs-generated duplicates. Dedup is centralised in Pass 3, which sees stated + obvious + all generated equally and fails open on error. **Lean LLM output schema (token/latency optimisation):** the generation LLM emits only the 9 fields it must author — `description`, `path`, `category`, `confidence_score`, `reasoning`, `priority`, `unpacks`, `depends_on`, `functional_area`. The deterministic fields (`req_id`, `source`, `tag`, `placement`, `strength`, `weight`, `testable`) are NOT emitted — `_validate_and_normalise` computes them, so the final stored object is schema-identical. `reasoning` is a single sentence covering both rationale and confidence; `confidence_reason` is back-filled from it in code (display-only, never scored), so frontend/types are unchanged. Cuts generation output tokens ~40% with zero accuracy/schema impact.
