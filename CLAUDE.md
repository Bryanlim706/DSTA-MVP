...
- **Two-pass generation — each output is a complete function with traversal path:**
  ...
  - **Pass 2 — INF domain inference (`category: "inf"`):** Reads `project_summary` + all stated functions. Generates across 7 domain-completeness angles that Pass 1 didn't cover: (1) recurring use, (2) workflow completeness / onboarding, (3) data management (export/import/bulk/archive), (4) domain standards (what a comparable app always has), (5) discoverability + help, (6) user control (settings/preferences), (7) overview/insight. Bold generation — generates at 0.50–0.70 for genuine gaps; breadth over silence to drive FA scoring. Never re-generates Step 1 stated, Step 2 obvious, or Pass 1 SOP functions.
