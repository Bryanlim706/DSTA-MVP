...
- **Step 5 / confirm.py absolute path fix** — `extract_to` is stored in the job JSON as a relative path (`uploads\job_id\extracted`). When FastAPI background tasks call `step5_app_crawler.run()`, the relative path must be resolved to absolute before use — `asyncio.create_subprocess_exec(cwd=...)` and `Path.exists()` both depend on the process CWD being the backend directory, which is not guaranteed. Fix: `confirm.py` now calls `.resolve()` before passing `extract_to` to the background task; `step5_app_crawler.run()` also calls `extract_to = Path(extract_to).resolve()` at the top of the function. This makes all subprocess `cwd` arguments absolute, fixing `boot_failed` in server context while the same code worked fine in direct script execution (where CWD was always correct).
