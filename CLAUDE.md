...
- Bootstrap heuristic from `step_3_5.project_context`: `project_type` + `frontend_framework` + `frontend_tooling` + `service_layout` determine the start command and port. Handled project types: `full_stack_web_app`, `full_stack_js` (same dispatch as `full_stack_web_app` — reads `service_layout`, calls `_find_frontend_dir` + `_npm_cmd` + `_backend_spec`), `frontend_only`, `backend_api_only`, `static_site`, `electron_app` (tries `npm run dev` from renderer dir; works for Electron Forge + Vite / electron-vite; falls to `boot_failed` static mode if no port opens). `mobile_app` intentionally returns empty (no browser UI). `monorepo`, `cli_tool`, `library`, `unknown` return empty → static-only mode.
...
- **Output validation:** `_parse_response` validates each suggestion: description non-empty, `suggestion_id` matches `FA-POS-\d+`, `priority` in {high, medium, low}, `l1a_connection` "null"/"none" strings coerced to `None`, missing `grounded_in` defaults to empty lists.
- **Array extraction (`_extract_array` + `_recover_truncated`):** Haiku reliably wraps the array in a ` ```json ` markdown fence despite the "no fences" instruction. Parser is fence/prose-tolerant: slices from the first `[` to the **last** `]` (dropping the trailing fence) and `json.loads` that; falls back to the raw tail; then to `_recover_truncated`, a brace-aware walk (tracks string state + object depth) that truncates after the last *complete top-level* object. **Why this replaced `rfind("},")`:** the old recovery matched the `},` that closes the nested `grounded_in` sub-object in the last element, producing malformed JSON → empty list. Combined with the fence (which already broke the primary `json.loads`), Step 7.5 silently returned `total_count: 0` with `error: null` on **every** run even though the LLM returned a full 4–8 item array. Fixed 2026-06-27.
