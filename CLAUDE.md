...
- **Step 11 Docker boot strategy** — Primary stack: Spring Boot (Maven/Gradle) + React (Vite/CRA/Angular/Next.js). Full boot pipeline:
  - **Pre-checks:** `_check_docker_available()` runs `docker info` first; `_port_in_use()` checks 8081/5181 are free.
  ...
  - **Vite proxy patching:** `_patch_vite_config()` rewrites `http://localhost:8xxx/9xxx` → `http://backend:8080` in vite.config.js/ts/mjs so proxy calls resolve inside the Docker network.
  - **Dockerfiles:** Backend: Maven multi-stage (`maven:3.9-eclipse-temurin-21-alpine` build stage + `eclipse-temurin:21-jre-alpine` runtime); fat-JAR detected via `jar tf "$j" | grep "^BOOT-INF/"` (works for single and multi-module projects). Gradle: prefers `./gradlew bootJar` over system `gradle`. Frontend: 5 templates — Vite (VITE_API_URL build arg), CRA (PORT=5174, REACT_APP_API_URL), Angular (npx ng serve --port 5174), Next.js (PORT=5174, NEXT_PUBLIC_API_URL), Generic (npm run dev || npm start). All standardise on internal port 5174.
  - **Compose:** DB service (MySQL:8/Postgres:16/MariaDB:lts) with healthcheck (`mysqladmin ping` / `pg_isready`) + `start_period` to absorb init time. Backend `depends_on: db: condition: service_healthy` prevents startup before DB ready. Frontend `build.args` passes all three API URL env var conventions.
  ...
  - **Output adds** `db_type` and `frontend_type` fields to Step 11 result schema.
